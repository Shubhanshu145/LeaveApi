...
export class UsersService {
    constructor(@InjectModel(User.name) private userModel: Model<User>,private s3Service:S3Service){}
    ...
    async updateProfileImage(email: string, newFilename: string, contentType: string): Promise<{ uploadUrl: string }> {
        ...
        const uploadUrl = await this.s3Service.generatePreSignedUrl(newKey, contentType);
